...
def get_train_and_test_dataloader(args, dataset_path, noise_ratio):
    train_dataset = datasets.load_train_dataset_from_file(label_noise_ratio=noise_ratio, dataset_path=dataset_path)

    train_dataloader = DataLoaderX(train_dataset, batch_size=args.batch_size, shuffle=True, num_workers=1, pin_memory=True)

    test_dataset = datasets.get_test_dataset(DATASET=args.dataset)

    test_dataloader = DataLoaderX(test_dataset, batch_size=args.batch_size, shuffle=False, num_workers=1, pin_memory=True)
...
def get_model(dataset, model_name, hidden_unit, device):
    ...
    model = LL_Layer(784, hidden_unit)
...
if __name__ == '__main__':
    ...
    parser.add_argument('-p', '--noise_ratio', default=1, type=float, help='label noise ratio')
    parser.add_argument('-m', '--model', default='SimpleFC_2', choices=['SimpleFC', 'SimpleFC_2', 'CNN', 'ResNet18'], type=str,
    ...
    parser.add_argument('--task', choices=['train', 'test'], help='what task to perform')
    ...
    if args.task == 'train':
        ...
        for test_number in range(args.start, args.end + 1):
            ...
            directory = f"assets/{args.dataset}-{args.model}/N=%d-Epoch=%d-RL-sgd-%d" % \
                        (args.sample_size, args.epochs, test_number)
            ...
            clean_train_dataloader, clean_test_dataloader = get_train_and_test_dataloader(args=args,
                                                                                          dataset_path=dataset_path,
                                                                                          noise_ratio=0)
            noisy_train_dataloader, noisy_test_dataloader = get_train_and_test_dataloader(args=args,
                                                                                          dataset_path=dataset_path,
                                                                                          noise_ratio=1)
            ...
            for hidden_unit in hidden_units:
                ...
                dictionary_n_path_1 = os.path.join(dictionary_path, "dictionary_%d_1.csv" % hidden_unit)
                ...
                with open(dictionary_n_path_1, "w", newline="") as fp:
                    writer = csv.DictWriter(fp, fieldnames=dictionary.keys())
                    writer.writeheader()

                # Train and evaluate the model
                train_and_evaluate_model(model, device, args, noisy_train_dataloader, clean_test_dataloader, optimizer,
                            criterion, dictionary_path=dictionary_n_path_1, checkpoint_path=checkpoint_path, phase=1)

                assert (0 == 1)

                # Most important section: train the classifier layer using the features layer trained from noisy labels
                model.features.requires_grad = False
                optimizer.param_groups[0]['lr'] = args.lr

                # Setup the dictionary file for n_hidden_unit
                dictionary_n_path_2 = os.path.join(dictionary_path, "dictionary_%d_2.csv" % hidden_unit)

                dictionary = {'Hidden Neurons': 0, 'Epochs': 0, 'Parameters': 0, 'Train Loss': 0, 'Train Accuracy': 0,
                              'Test Loss': 0, 'Test Accuracy': 0, 'Learning Rate': 0, 'Time Cost': 0, 'Date-Time': 0}

                with open(dictionary_n_path_2, "w", newline="") as fp:
                    writer = csv.DictWriter(fp, fieldnames=dictionary.keys())
                    writer.writeheader()

                train_and_evaluate_model(model, device, args, clean_train_dataloader, clean_test_dataloader, optimizer,
                            criterion, dictionary_path=dictionary_n_path_2, checkpoint_path=checkpoint_path, phase=2)
    # Testing
    elif args.task == 'test':
        ...
        for test_number in range(args.start, args.end + 1):
            ...
            for hidden_unit in hidden_units:
                ...
                dictionary_path = os.path.join(directory, "dictionary/dictionary_%d_1.csv" % hidden_unit)
